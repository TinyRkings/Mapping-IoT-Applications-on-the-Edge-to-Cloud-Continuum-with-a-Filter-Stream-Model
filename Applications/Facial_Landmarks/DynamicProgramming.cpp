...
int main(){
	int stage =5;
	int P_adjust=1294;
	int Power_local1[]={2872, 2564,2468,2303};//mW
	int first_power= 2066;


	
	//int time_local[2][9];
	int first_time= 18163;
	int first_time2=first_time/1000;//ms
	int time_local1[] = {15288,21584,65463350,112};//us
	int time_local2[] = {15288,21584,65463350,112};//us
	
	int first_timepc= 18376;
	int time_pc1[] ={1441,9751,17423864,19};
	
	int tranmiss_size[]={640*480, 640*480, 640*480+32, 640*480+32+56};
	int first_transmiss=640*480*3;
	
	//number of services (or number - 1)
	int Tcost[4][4];//m*m (middle layer)
	int FirstTcost[4];//Tcost for first layer
	int LastTcost[4];// Tcost for last layer
	int Ecost[4];//m
	...
	for(int ww=0;ww <num_services; ww++)
	{
		FirstTcost[ww]= Sumlocaltime(time_local1, 0,ww,tranmiss_size[ww],bandwidth1) * P_adjust * weight2;
		Ecost[ww] = SumEnergy(Power_local1,time_local1,0,ww,tranmiss_size[ww],bandwidth1) * weight1;
		//printf("Ecost:%d\n", Ecost[ww]);
	}
	//last layer from ww to 3: Tcost (PC)
	for(int ww=0;ww <num_services; ww++)
	{
		LastTcost[ww]= Sumlocaltime(time_pc1, ww, num_services-1,0,bandwidth2) * P_adjust * weight2;
	}
	//number of services, number of layers
	int cost[4][3];//m==4, n==3
	int cut[4][3];
	for(int i=0;i<num_services;i++)
		for(int k=0;k<3;k++)
		{
	...
	for(int i=num_services-1;i>-1;i--)
		for(int k=0;k<3;k++)
		{
	...
	for(int ww=num_services-1;ww>-1;ww--)
		for(int i=0;i<3;i++)
			printf("%d, %d, %d\n",ww, i, cost[ww][i]);
